...
/**
 * Function to read ashwl colors and update the theme
 */
async function sync_ashwal() {
    try {
        // Glide's internal FS API can read local files without CORS issues
        const wal_raw = await glide.fs.read("/home/ashwin/.cache/ashwal/colors.json", "utf8");
        ...
        console.log("Glide: ashwal colors synced successfully.");
    } catch (err) {
        console.error("Glide: ashwal sync failed:", err);
    }
}
 ...
 */
glide.autocmds.create("ConfigLoaded", sync_ashwal);
